...
- 回测：`python run.py backtest JP --universe broad --macro all`（`macro` / `sector` / `events` 可单测）；`optimize` 同样支持
- 模拟盘 / `paper` / `signal` / `daemon` 默认启用；`sim.json` 里 `use_macro / use_sector_tilt / use_event_window` 可关；命令行 `--no-macro`
