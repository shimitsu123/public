...
def build_unified_data() -> dict:
    ...
    return {"generated": dt.datetime.now().strftime("%Y-%m-%d %H:%M"), "mode": "unified", "sim": sim,
...
def render_unified_html(d: dict) -> str:
    ...
    todo_html = (f'<h3>09:00 日本开盘（寄付）</h3><ul>{rows(td.get("JP", []), "JP")}</ul>'
                 f'<h3>日间 换汇（楽天：片道 25 銭/USD）</h3><ul>{rows(td.get("FX", []), "FX")}</ul>'
    ...
    for m, e in (d.get("extras") or {}).items():
        ...
        mk.append(f"<dt>{'日本' if m == 'JP' else '美股'}</dt><dd>状态 {escape(str(r.get('label', '')))}；新仓倍数 ×{r.get('final_mult')}；"
                  f"牛熊分界 {escape(str(bb.get('state', '?')))}（自 {escape(str(bb.get('since', '?')))}，翻转价位 {bb.get('level')}）；"
    ...
    return _PAGE.format(
        ...
        rules=escape(f"个股 {cfg.get('max_positions')}×{int(float(cfg.get('position_pct', 0)) * 100)}%（日本 + 美股合计、一起排名：日本 → 美元已够的美股 → 要换汇的美股）；"
