...
class TachibanaBroker(BaseBroker):
    ...
    def quotes(self, tickers: list[str]) -> dict[str, float]:
        """批量取现在值（1 次最多 120 只）。取不到的（开盘前 / 休市 / 暂时拒绝时是 ""）跳过，不让一只票拖垮整轮。"""
        s = self.spec
        out: dict[str, float] = {}
        for i in range(0, len(tickers), s.max_quote_codes):
            ...
            for t in chunk:
                v = (by.get(self._code(t)) or {}).get(s.r_price)
                try:
    ...
    def _confirm(self, o: Order) -> Order:
        ...
        while o.filled_qty < o.qty and _mono() < deadline:
            _sleep(1.0)
            try:
                r = self._call(s.clm_order_detail,
                               **{s.f_order_no: o.broker_id, s.f_order_date: o.extra.get("order_date", "")})
            except Exception as e:                        # noqa: BLE001
            ...
            try:
                ...
                pass
            o.extra["broker_status"] = r.get(s.r_status_code, "")
        if o.filled_qty == 0:
            o.status = "SENT"
    ...
    def cancel(self, client_id: str) -> bool:
        ...
        s = self.spec
        try:
            if self.creds is None or not self.creds.second_password:
                raise BrokerError("缺少第二暗証番号")
            self._call(s.clm_cancel_order, idempotent=False,
                       **{s.f_order_no: ref[0], s.f_order_date: ref[1], s.f_second_pw: self.creds.second_password})
            return True
        except Exception as e:                            # noqa: BLE001
