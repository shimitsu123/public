...
def in_sample(d: dict, x: dict) -> dict:
    ...
    for name, s in (("W", w), ("A0", a0)):
        q = TH.expanding_pct(s) >= 0.9
        out[f"{name}_hits"] = [sum(bool(q.loc[:p].tail(61).any()) for p in peaks), len(peaks)]
    return out
...
def main() -> int:
    ...
    if fp_log.exists() and len(pd.read_csv(fp_log)):
        ...
        lines += [f"\n## 前瞻（{r['first']} 起，{r['days']} 天，其中结果已知 {r['known']} 天、事件日 {r['event_days']} 天）",
                  ...
                  f"- 警戒日 {r['alert_days']} 天；警戒后 60 日内真的跌 ≥10% 的比例 {fmt(r['alert_hit'])}",
                  "- 前瞻期内 ≥10% 下跌：" + ("；".join(f"{e['peak']} 高点（W 事前警戒 {'有' if e['W_alert'] else '无'}，"
                                                     f"A0 {'有' if e['A0_alert'] else '无'}）" for e in r["episodes"]) or "还没有"),
                  f"\n判定（事先规则）：{r['decision']}"]
    else:
        lines.append("\n还没有前瞻记录（第一次日报运行后开始）。")
    try:
        ...
        lines += ["\n## 样本内参考（偏乐观：两个因素是看过 2011 年后结果才挑的，不作判定依据）",
                  "| 指标 | 跌≥10% AUC 1995–2010 / 2011– | 跌≥15% AUC 1995–2010 / 2011– | 下跌前 60 日内到过自身 90 分位 |",
                  "|---|---|---|---|"]
        for name, lab in (("W", "W（金银比 + 商品波动）"), ("A0", "A0 现行威胁指数")):
            a10, a15, h = ins[f"{name}_10"], ins[f"{name}_15"], ins[f"{name}_hits"]
