...
## 待办清单（2026-09-25 合并；用户说「从①开始」时按顺序一次做一项，做完报告结果再做下一项）
🤖 = Mac 上的 Claude 可以直接执行　✋ = 要用户本人在 Mac 前操作或决定　☁️ = 只能在云端会话（claude.ai/code）做（改代码、改例行任务）。
做完的项由云端会话从这里删掉（Mac 上不改仓库文件）。

**A. 回家后先做（最好在 2026-09-28 07:40 JST 之前）**
- ① 🤖 拉代码并读规则：`git -C ~/qbreak-src pull --ff-only`，然后读 `~/qbreak-src/CLAUDE.md` 与本文件并遵守。
  若报「untracked working tree files would be overwritten … CLAUDE.md」= 本机有同名文件 → ✋ 同意后把它改名备份再拉
- ② 🤖✋ 桌面链接 + 打开页面：`bash ~/qbreak-src/quant_breakout/scripts/liveu.sh desktop`；弹出「终端想访问桌面文件夹」→ ✋ 点允许；
  确认桌面有 `qbreak模拟操盘.html`、双击能打开（9/28 之前显示「还没有开始」是正常的）。云端验证不了这一项
- ③ 🤖（可选）试跑预览：`bash ~/qbreak-src/quant_breakout/scripts/liveu.sh trial`（1〜5 分钟，打开试跑页面，看「牛熊：现在处于哪个阶段」）
- ④ 🤖✋ 检查：`launchctl list | grep qbreak` 有 `com.qbreak.liveu.paper`；`date +%z` 是 `+0900`；
  ✋ 系统设置 → 通知 → 允许「スクリプトエディタ / Script Editor」（通知以它的名义弹出）
- ⑤ ✋ 决定页面每天早上自动弹出要不要保留（不要 → 🤖 `touch ~/.qbreak/home/NO_OPEN`）
- ⑥ ✋（可选）个人偏好（沟通方式以外的个人信息）只写本机 `~/.claude/CLAUDE.md`，不进仓库

**B. 要用户决定，在云端做（Mac 上的 Claude 改不了例行任务，也不改仓库）**
- ⑦ ✋☁️ 日报例行任务的汇报第 ③ 项是否改成先写「阶段 + 百分比」（日报页面本身已显示；改汇报文字要在例行任务自己的会话里改，
  或删了重建 → 例行任务 ID 会变）
- ⑧ ✋☁️ 上线门槛提案（见下面「路线图」4）采用 / 改数字 / 不要 → 定了由云端会话写进本文件

**C. 按日期**
- ⑨ 2026-09-28（周一）07:40〜08:30 第一次正式运行：页面自动打开了吗、通知弹了吗、「与云端一致」吗 → 不一致或标红就问原因
  （去哪查见下面「用户常问的，去哪里查」）
- ⑩ 9/28〜10/2 第一周：每天看一致性，把解释不了的差异记下来
- ⑪ 2026-10-12（周一）09:56 季度复核：看汇报第一行有没有「需要用户确认」
- ⑫ 立花开户之后：按「路线图」3 逐步做（`ARM` 只在用户明确要求时创建）
