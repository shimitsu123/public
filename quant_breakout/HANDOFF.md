...
- 买点质量分的前向记录（2026-09-26 登记，用户要求）：2026-09-28 起云端 sim-day 自动把每个信号的分数追加到
  ...
  已平仓 100 / 200 / 400 笔时才判定（约 3 / 6 / 11 年）。加进季度例行任务需要用户确认（待办 ⑮）
...
- 前向记录扩大到 TOPIX 1000（2026-09-26 追加登记 32c16cb）：日経225 以外 716 只的信号记到 `var/out/score_forward_wide.csv`；
  合并样本已平仓 200 / 400 / 800 笔时判定（约 1 / 2 / 3.5 年）。同一天的「没参与过设计的股票」检验：F2 分数在别的股票上不成立
...
- ⑮ ✋☁️ 要不要把 `python scripts/score_forward.py --review`（买点质量分前向记录的复核，含扩大池）加进季度例行任务 → 用户确认后在云端改
