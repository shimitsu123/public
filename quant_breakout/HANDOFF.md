...
- 资金规模 / 名额分配（2026-09-26，事先登记 115c662）：¥100 万时「4 个名额 + 一手放宽 50%」（U2：按名额买不到一手、但一手 ≤ 权益 50% 且现金够 → 买一手）
  按登记规则通过（主窗口 Calmar 0.453 → 0.529，两半与 20 年都不差）；2026-09-26 用户确认启用，**2026-09-27 生效前用户确认撤回**
  （`var/sim.json` 的 `unified.one_lot_cap_pct: 0` = 关；理由：事后诊断用 J-Quants 行情、真实一手时一手放宽反而更差 —— 今天的日経225
  Calmar 0.372 → 0.337、时点 TOPIX 500 0.343 → 0.203，`var/out/ml_posthoc.md`；capital_study 用的是 yfinance 行情 → 效果不稳健）。
  资金到 ¥300 万左右以后「一手太贵」基本消失（`var/out/capital_study.md`）
...
- 全部因子的组合网罗（2026-09-26，事先登记 055649e）：47 个因子（市场层 30 + 个股层 17）的单个 / 两两 / 三个组合共 42,749 个 × 7 个目标
  （①日経225 突破交易赚钱、②日経 / S&P500 牛→熊・熊→牛、③之后 60 日跌 ≥10%）；随机对照 20 次 + BH 校正 → 7 个目标都是 0 个显著
  （不校正的话 ① 有 4,589 个组合「看起来有效」），进前向观察名单 0 个，维持现行（`var/out/factor_combo_study.md`）
- J-Quants 10 年时点股票池 重新检验 / 重训（2026-09-26，事先登记 f30103b，运行中的报错修正 60e4b18）：无幸存者偏差的时点 TOPIX 500 / 1000
  上现行参数的 S0C2 年化 12.53% / 3.59%（今天的日経225 同口径 13.41%）→ 换成 TOPIX 1000 明显更差；重训（43 个单项，箱体天数 90 等）与
  宏观倍数候选（熊市新仓减半）都没过验证期 → 维持现行参数与日経225 股票池（`var/out/pit_retrain_study.md`、`qbreak/pit_data.py`）
- 全部因子的机器学习（2026-09-27，事先登记 3fc4ed3）：72 个特征（技术面、现行买点因子、基本面、利率 / 汇率 / 国债 / 油价敏感度、市场层）、
  入场与出场模型（岭回归 / 提升树，每年重训，纯 numpy `qbreak/ml.py`）→ 预测力弱且不稳定（入场 IC 验证期 +0.023 不显著、留出期 +0.053；
  出场 AUC 0.51〜0.55）；按登记规则 K2（低分突破信号不买）通过 = 提议，但事后诊断显示它只在一手放宽 50% 下成立（`var/out/ml_study.md`、
  `var/out/ml_posthoc.md`）→ 一手放宽撤回后 **K2 不采用，模拟盘未改**
- 多周期（日 / 周 / 月线，2026-09-27，事先登记 fe49536，显示修正 ef72213；研究路线图 `RESEARCH_PLAN.md` 的 R1）：所有股票「日周月都向上」
  之后 20 日没有超额；趋势类规则与周线突破系统 2019〜2023-09 更差、2023-10〜 更好（时代依赖）；两期一致的只有周线量比；周 / 月线见顶特征几乎不起作用
  （94% 的离场是日线死叉）；C1〜C5 全部没过 → **维持现行**（`var/out/mtf_study.md`）。事后诊断：同一套设定下「个股一笔不买、只有 1655」
  2017〜2026 年化 14.17% / Calmar 0.424，比现行（12.55〜13.41% / 0.343〜0.372）好 → 下一项 R2a「个股层值不值得、占多少」（20 年也要看；`var/out/mtf_posthoc.md`）
- K 线 / 图形（2026-09-27，用户要求；研究路线图 R8）：用户图中的上影陽線「第二天大概率涨」不成立（事先登记 78c7b3d，4 个时期 48〜51%）；
  经典形态、二番底 / 三尊、相似 K 线检索、K 线提升树都没有预测力（探索记录 `var/out/candle_explore_1〜12.md`，只用 2017〜2021）；
  探索期最好的缩量押し目（时点 TOPIX 1000）登记检验 C1〜C5 全部没过验证期（事先登记 1e9bb57）→ **维持现行**（`var/out/candle_study.md`、事后 `candle_posthoc.md`）
- 个股层值不值得（2026-09-27，研究路线图 R2a，事先登记 f50298c）：主判定 2006〜2016「只有 1655」Calmar 0.259 vs 现行 0.217（+0.042 < 门槛 0.05），
  2017〜2026 0.424 vs 0.372 → 没通过、**维持现行**；但两个年代都是只有 1655 略好、回撤更浅（`var/out/layer_study.md`）
- 周线放量的突破（2026-09-27，事先登记 9990cba）：**W2「只做周线量比 ≥ 1.0 的突破」按登记规则通过 = 提议**（主判定 2006〜2016 Calmar 0.217 → 0.286、
  回撤 −32.32% → −28.75%、胜率 38.0% → 43.9%；2017〜2026 0.372 → 0.389）；交易少约 40%、好处主要在坏年份；随机少做同样比例的对照：2006〜2016 30 次都不如 W2、2017〜2026 与随机差不多（`var/out/wvol_placebo.md`）；日経225 以外 666 只的 2006〜2016 逐笔没有差别、组合 +0.024（`var/out/wvol_wide.md`）；**模拟盘未改**，等用户确认（`var/out/wvol_study.md`）
- 放量突破的加强版（2026-09-27，事先登记 09d859a）：V1「W2 + 突破日 3 倍量」、V3「只看突破日 3 倍量」通过对现行的门槛（含随机对照），但不比 W2 好 0.05 → **提议仍是 W2**（`var/out/vthrust_study.md`）；突破仓位加均线离场只做了探索、不一致、不登记（`var/out/exit_explore.md`）
- 市场宽度确认（2026-09-27，事先登记 79887d4）：A50（日経225 成分站上 50 日线的比例）≥ 0.6 / 0.7 才买、或再加 W2 → 全部没通过、**维持现行**（`var/out/breadth_study.md`）；同一轮还探索了投资主体买卖、配当権利取り、均线离场、按行情切换离场 → 探索里就不一致、不登记（sim_changes.md）
- 核心仓位的搭配（2026-09-27，研究路线图 R10，事先登记 1fd9ec8）：美股熊市换黄金 / 美国长债、常配黄金 20% → 没通过、**维持现行（熊市留现金）**；2008 年日元升值，熊市拿外币资产按日元计也亏（`var/out/refuge_study.md`）
...
- 影子账户（2026-09-26 用户要求并确认，事先登记 `scripts/shadow_account.py`）：规则以外的「判断型」选股单独记一个模拟账户（¥100 万、9/28〜12/24、同一费用 / 成交口径；日経225 + 1655 / 1329，个股 ≤ 4 只、单只 ≤ 35%、不加杠杆）：例行任务「影子账户判断」
  （每个交易日 07:45 JST，发到日报例行任务同一个会话）等当天日报入库 → `shadow_account.py step` → Claude 按日报 / 市场风险报告判断（09:00 之前）→ `decide`；和规则账户逐日对比（`var/out/shadow_equity.csv`，只追加）；12-24 收盘后按事先写定的四条评估。只记录，不影响模拟盘与交易
...
- ⑨ 2026-09-28（周一）07:40〜08:30 第一次正式运行：页面自动打开了吗、通知弹了吗、「与云端一致」吗 → 不一致或标红就问原因
  （去哪查见下面「用户常问的，去哪里查」）
- ㉓ ✋ 决定要不要启用 W2「只做周线量比 ≥ 1.0 的突破」（wvol_study 通过的提议；启用 = 改信号代码 + 测试 + sim_changes，用户确认后做）
...
- ㉑ 2026-12-25 早上：影子账户（判断型）3 个月评估 —— 例行任务「影子账户判断」在 12-25 07:45 JST 自动运行
...
- 股票池是 2026-09 时点的成分股 → 回测有幸存者偏差（偏乐观），只适合比较方案之间的相对差异；回测收益是税前、未计 NISA。
  ...
  整套 S0C2 同口径（J-Quants 行情、真实一手）：今天的日経225 年化 13.41% vs 时点 TOPIX 500 12.53%（差 0.88 pp / 年，含股票池不同；`var/out/pit_retrain_study.md`）
