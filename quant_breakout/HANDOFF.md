...
- 资金规模 / 名额分配（2026-09-26，事先登记 115c662）：¥100 万时「4 个名额 + 一手放宽 50%」（U2：按名额买不到一手、但一手 ≤ 权益 50% 且现金够 → 买一手）
  按登记规则通过（主窗口 Calmar 0.453 → 0.529，两半与 20 年都不差）；**2026-09-26 用户确认启用**（`var/sim.json` 的 `unified.one_lot_cap_pct: 0.5`，
  2026-09-28 起云端模拟盘与 Mac 执行器同一设定；代价：单只最多约占权益 50%）。资金到 ¥300 万左右以后「一手太贵」基本消失（`var/out/capital_study.md`）。
  ⚠ 2026-09-27 事后诊断（J-Quants 行情、真实一手；capital_study 用的是 yfinance 行情）：一手放宽反而更差（今天的日経225 Calmar 0.372 → 0.337，时点 TOPIX 500 0.343 → 0.203；
  `var/out/ml_posthoc.md`）→ 要不要撤回由用户决定（待办 ㉒）
...
- ⑨ 2026-09-28（周一）07:40〜08:30 第一次正式运行：页面自动打开了吗、通知弹了吗、「与云端一致」吗 → 不一致或标红就问原因
  （去哪查见下面「用户常问的，去哪里查」）
- ㉒ ✋ 2026-09-28 06:57 JST 之前（最好）：决定「一手放宽 50%」要不要撤回（事后诊断：J-Quants 口径下更差，`var/out/ml_posthoc.md`）；
  撤回 = `var/sim.json` 的 `unified.one_lot_cap_pct` 改回 0（或删掉）并记进 sim_changes；ml_study 的 K2 提议要不要用（不建议，见 sim_changes）
...
- 个股：日経225 股票池的突破信号（`qbreak/strategy.py`，参数 `qbreak/config.py` + `var/best_params_JP.json`）；
  4 个名额 × 权益 25%，单只 ≤ 34%；按名额买不到一手时，一手 ≤ 权益 50%（× 新仓倍数）且现金够 → 买一手（一手放宽，2026-09-28 起，
  `var/sim.json` 的 unified.one_lot_cap_pct = 0.5）→ 单只最多约占权益 50%；新仓倍数 = 量化状态层 × 宏观层 × 板块倾斜；事件窗口关闭；
