...
- 周线放量的突破（2026-09-27，事先登记 9990cba）：**W2「只做周线量比 ≥ 1.0 的突破」按登记规则通过 = 提议**（主判定 2006〜2016 Calmar 0.217 → 0.286、
  回撤 −32.32% → −28.75%、胜率 38.0% → 43.9%；2017〜2026 0.372 → 0.389）；交易少约 40%、好处主要在坏年份；随机少做同样比例的对照：2006〜2016 30 次都不如 W2、2017〜2026 与随机差不多（`var/out/wvol_placebo.md`）；**模拟盘未改**，等用户确认（`var/out/wvol_study.md`）
